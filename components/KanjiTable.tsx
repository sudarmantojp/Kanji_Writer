...
export default function KanjiTable({
  ...
  const setCanvasRef = useCallback(
    (globalIndex: number) => (handle: KanjiCanvasHandle | null) => {
      if (handle) {
        canvasRefs.current.set(globalIndex, handle)
      } else {
        canvasRefs.current.delete(globalIndex)
      }
    },
    []
  )
  ...
  const goToPage = (p: number) => {
    setPage(p)
    // deactivate row when changing page
    onActivateRow(-1)
  ...
  return (
    <div className="flex flex-col gap-0">
      {/* ── Table ── */}
      <div
        className="overflow-x-auto rounded-t-xl border"
        style={{ borderColor: '#d5cec4', borderBottom: 'none' }}
      >
        <table className="w-full border-collapse" style={{ minWidth: 700 }}>
          <thead>
            <tr style={{ background: 'oklch(0.93 0.014 50)' }}>
              ...
              <th
                className="text-center font-semibold text-xs py-3 px-4 whitespace-nowrap"
                style={{
                  color: '#5a4a3a',
                  width: '1%',
                  borderBottom: '2px solid #d5cec4',
              ...
              <th
                className="text-left font-semibold text-xs py-3 px-4 whitespace-nowrap"
                style={{
                  color: '#5a4a3a',
                  borderBottom: '2px solid #d5cec4',
                  letterSpacing: '0.04em',
            ...
            {pageData.map((entry, localIndex) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Kanji — with Radix tooltip showing reading on hover */}
                  <td
                  ...
                  {hasMeaning && (
                    <td
                      ...
                      style={{ color: '#4a3a2a', verticalAlign: 'middle', maxWidth: 160 }}
                    >
                      {entry.meaning ?? '—'}
                    </td>
                  )}
                  ...
                  {/* Clear button */}
                  <td className="text-center py-2 px-3" style={{ verticalAlign: 'middle' }}>
                    <button
                      onClick={(e) => {
                        e.stopPropagation()
                        clearRow(globalIndex)
                      }}
                    ...
                    >
                      {isCleared ? <Pencil size={14} /> : <Eraser size={14} />}
                    </button>
                  </td>
                </tr>
              )
      ...
      <div
        ...
        style={{
          background: 'oklch(0.93 0.014 50)',
          border: '1px solid #d5cec4',
          borderTop: '2px solid #d5cec4',
        }}
      >
        {/* Info */}
        ...
        <div className="flex items-center gap-1">
          ...
          {Array.from({ length: totalPages }, (_, i) => {
            ...
            // show ellipsis logic: always show first, last, current ±1
            const show =
              i === 0 ||
              i === totalPages - 1 ||
              Math.abs(i - page) <= 1
            ...
            if (!show) {
              ...
              if (i === 1 && page > 2) {
                return (
                  <span key={`ellipsis-start`} className="text-xs px-1" style={{ color: '#9a8a7a' }}>
                    …
                  </span>
                )
              }
              if (i === totalPages - 2 && page < totalPages - 3) {
                return (
                  <span key={`ellipsis-end`} className="text-xs px-1" style={{ color: '#9a8a7a' }}>
                    …
                  </span>
                )
              }
