...
const KanjiCanvas = forwardRef<KanjiCanvasHandle, KanjiCanvasProps>(
  ({ kanji, isEraseMode, isActive, onActivate, brushSize = 3 }, ref) => {
    ...
    /* Scale mouse/touch coords to canvas internal resolution */
    ...
    /* Auto-erase on hover when erase mode + this row active */
    useEffect(() => {
      if (isEraseMode && isActive && isCursorOver) clearCanvas()
    ...
    const draw = useCallback((e: MouseEvent | TouchEvent) => {
      ...
      if (isEraseMode) {
        ...
        ctx.beginPath()
        if (lastPos.current) { ctx.moveTo(lastPos.current.x, lastPos.current.y); ctx.lineTo(pos.x, pos.y) }
        else { ctx.moveTo(pos.x, pos.y); ctx.lineTo(pos.x + 0.1, pos.y + 0.1) }
        ctx.stroke()
      }
      lastPos.current = pos
    ...
    return (
      ...
      >
        <canvas
          ...
          onMouseLeave={() => { setIsCursorOver(false); stopDraw() }}
          style={{
            display:     'block',
            width:       '100%',
            height:      'auto',
            ...
            borderRadius: 6,
            border:  isActive ? '2px solid oklch(0.55 0.18 30)' : '1px solid #d5cec4',
            cursor:  isEraseMode && isActive ? 'cell' : isEraseMode ? 'default' : 'crosshair',
            touchAction: 'none',
            background:  '#fff',
            boxShadow:   isActive
